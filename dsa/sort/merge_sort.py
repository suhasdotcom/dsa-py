...
def merge_sort(a_iterable):
    """
    Sorts the iterable using merge sort in ascending order
    ...
    return merge_iterables(merge_sort(a_iterable[0:a_iterable_length // 2]),
                           merge_sort(a_iterable[a_iterable_length // 2: a_iterable_length]))


def merge_iterables(a_iterable_1, a_iterable_2):
    """
    Merges two iterables using finger sorting, ascending currently

    :param a_iterable_1: the first iterable
    :param a_iterable_2: the second iterable
    :return: ascending merged iterable
    """
    ...
    for counter in range(0, final_new_iterable_length):
        if i < a_iterable_1_length and j < a_iterable_2_length:
            if a_iterable_1[i] < a_iterable_2[j]:
